...
| 域名 | `kb.liii.in` / `kb.tokenwave.cloud` / `uat.tokenwave.cloud` / `skillhub.tokenwave.cloud` / `aihub.tokenwave.cloud` / `kanban.tokenwave.cloud` / `studio.tokenwave.cloud`(同 IP,各自证书) |
...
**edge 拆分的意义**:日常前后端部署只重建各自内网容器,edge 不动 → aihub/skillhub/kanban/studio 全程无感;edge 内所有 upstream 用 `resolver 127.0.0.11 + set $var` 延迟解析,容器重建换 IP 10s 内自愈、任一容器缺失不挡 edge 启动。只有 `edge/` 或 `docker-compose.yml` 变更才重建 edge(全域名闪断几秒)。

`kanban.tokenwave.cloud` 是独立 Plane 栈:`/opt/kanban/docker-compose.yml` 启动 `web/api/worker/beat-worker/migrator/admin/space/live/plane-db/plane-redis/plane-mq/plane-minio/proxy`。`edge` nginx 持 HTTPS 证书,反代到 Plane 自带 Caddy `plane-proxy:80`(external network `kb-system_default` 别名),不要再指向旧的 `planka:1337`。
...
```
                 ┌─────────────────────────────────────┐
                 │  nginx (80/443) — edge 容器          │
                 │  按域名分流:kb→frontend uat→uat     │
                 │  skillhub/aihub/kanban/studio→各自   │
                 └──────────────┬──────────────────────┘
                                │
                 ┌──────────────▼──────────────────────┐
                 │  frontend 容器(:80 内网,kb dist)   │
...
| `mcp.py` | `/api/mcp` | MCP Streamable HTTP server(对外 AI 工具调用)。14 个 tool:12 读(ask_kb/search_kb/list_projects/get_project_status/list_outputs/get_output/get_brief/get_smart_advice/list_documents/get_document/list_meetings/get_meeting)+ 2 写(generate_output/create_meeting_from_text)。全 per-user 项目隔离;写工具校 write 权限。kind 列表从 `api.outputs.KIND_TITLES` import,不再手抄 |
...
> ⚠️ **曾经的隐藏第 4 处:`backend/api/mcp.py`**。MCP 的 `list_outputs` / `get_project_status` 历史上自己硬抄了一份 5 个 kind 的列表,长期没跟上 → 整条 LTC 链对 MCP 不可见(2026-06-25 修)。现在 mcp.py 直接 `from api.outputs import KIND_TITLES`,**不再是手动同步点**。新加 kind 仍只改上面三处 + output_tasks + runner,MCP 自动跟上。触发生成也复用 `api.outputs.enqueue_generation()`(HTTP `/generate` 与 MCP `generate_output` 共用),不再二次硬抄 KIND_TO_TASK→task 映射。
...
线性阶段 Tab 之外的**第二种项目推进形态**:把 13 种交付物 + 4 个资料桶(项目资料 / 会议纪要 / Brief / 网络调研)+ 自定义输入(手写备注 / 网页 / 文件)摆成 React Flow 节点,可拖拽自由排布、连线、任意顺序/并行运行、双击进现有阶段工作区。与线性界面**并存**,不替换。**Level B 已落地** —— 连到生成节点的自定义输入会真正喂进该次生成(见下)。
...
- **新旧 UI 都启用 + 主题自适应**:`App.tsx` 路由 `console/projects/:id/canvas` 无条件注册 + 懒加载(~55KB gzip,不进主包);入口「画布」按钮在新(深色 `redesign/console/ConsoleProjectDetail`)和旧(浅色 `pages/console/ConsoleProjectDetail`)都加了。画布按 host(uat)/`?ui=new` 判 `isDark`,颜色全走 `.kb-canvas` 上的 `--cv-*` 变量(深/浅两套)。
...
- **种子图**:空项目首载时前端从 stage-flow + 依赖边生成种子图(资料桶 + 已有产物的生成节点;依赖边精简成主链),用户首次保存才落库。双击生成节点深链 `?stage=&sub=` 回阶段工作区(`ConsoleProjectDetail` 消费 `?sub=` 一次即抹掉)。
- **自定义输入节点(`InputNodes.tsx`:note/webpage/file)**:可多份(随机实例 id),内容存进 `GraphNode.data`(text/url/docId);file 走现有 `/documents/upload`。`workflow_canvas.py` 的 `GraphNode` 加了 `data: dict | None`。
- **Level B 连线喂进生成**:运行节点前前端先存画布;后端 `runner.py::_load_canvas_connected_docs(project_id, kind)` 读 workflow_canvas,沿入边收集连到 `gen_<kind>` 的 note/webpage/file → 合成文档并入 `_load_ctx` 的 `docs_by_type`(D 类源,按 doc_id 去重),**一处接入覆盖全部 11 kind**(agentic `_build_sources_index` 与 research `format_docs_for_report` 都按 doc_type 渲染)。webpage 用 httpx best-effort 抓正文(超时/去标签/限长/防内网)。`DOC_TYPE_LABELS` 加 `canvas_note/webpage/file`。
- **连线 / 布局**:连线可重连(`onReconnect`+`reconnectEdge`);「整理布局」用 **elk**(`elkLayout.ts`,layered+ORTHOGONAL,动态 import ~1.4MB 仅点时下载)正交布线绕开节点,`OrthEdge` 自定义边沿 elk 折线点画圆角,拖动节点后回退 smoothstep。
...
- ✅ MCP 扩到 14 个 tool(8→14):补全文档全文 / 会议 / 智能建议只读 + generate_output / create_meeting_from_text 写工具,修复 kind 列表漂移(改用 KIND_TITLES import),鉴权策略不变(2026-06-25,见 § 4 mcp.py 行 + § 6.8)
