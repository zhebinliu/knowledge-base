...
```bash
# 本地改 + scp
scp -i ~/.ssh/id_rsa_github_deploy main.go liu@34.42.241.99:/opt/aihub-tap/
# 服务器重启容器(go run 会重编译,~3 秒)
ssh -i ~/.ssh/id_rsa_github_deploy liu@34.42.241.99 "cd /opt/aihub-tap && sudo docker compose restart aihub-tap"
```
...
| 2026-06-22 → 6-23 | **tingya 自主修复**:fixer `max_tokens` 4096 → 8192(灰度 1 天后全量),截断率从 1.5%(17/1106) 降到 **0.1%(1/882)**。无需运维介入,记一笔正向案例 |
