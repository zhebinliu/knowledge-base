...
def inspect_skill(storage_path: str) -> dict:
    """完整跑一遍质检,返回可入库 dict。"""
    t0 = time.time()
    static = static_check(storage_path)

    if not settings.llm_api_key:
        # 没配 LLM key:纯静态模式,只给 format 分,其他维度按基础结构估算
        score = static["format_seed_score"] * 4  # 全凭 format 一项放大,粗略
        score = max(0, min(100, score))
        return {
            "score": score,
            "verdict": _verdict(score),
            "summary": "未配 LLM key,本次为纯静态打分(仅基于格式合规)。",
            "dimensions": {
                "format":    {"score": static["format_seed_score"], "comments": "静态规则评分"},
                "trigger":   {"score": 0, "comments": "未启用 LLM 评分"},
                "content":   {"score": 0, "comments": "未启用 LLM 评分"},
                "structure": {"score": 0, "comments": "未启用 LLM 评分"},
            },
            "suggestions": static["static_issues"] + [
                {"severity": "low", "area": "format", "message": "管理员配置 SKILLHUB_LLM_API_KEY 后可开启完整评分"}
            ],
            "llm_model": None,
...
请严格按上面定义的 JSON schema 返回评分。"""
    ...
    try:
        if settings.llm_provider == "anthropic":
            parsed, raw = _call_anthropic(user_prompt)
        else:
            parsed, raw = _call_openai_compat(user_prompt)
    except httpx.HTTPError as e:
        return {
            "score": 0,
            "verdict": "fail",
            "summary": f"LLM 调用失败: {type(e).__name__}: {e}",
            "dimensions": {"format": {"score": static["format_seed_score"], "comments": "静态规则评分"}},
            "suggestions": static["static_issues"],
    ...
    # 合并静态 issues 到 suggestions
    sugg = list(parsed.get("suggestions") or [])
    for it in static["static_issues"]:
        if not any(s.get("message") == it["message"] for s in sugg):
            sugg.append(it)
