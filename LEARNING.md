...
### 6.7 Tiptap v3 + React 18 必须 `immediatelyRender: false`

**症状**:产物在线编辑器(MarkdownEditor)首次打开就报 React 最小化 error #310
("Rendered more hooks than during the previous render")。开发模式下也偶发。

**真因**:Tiptap v3 默认 `immediatelyRender = true`,在 React render 阶段
**同步**创建 editor 实例 + 立即触发首次 `onUpdate` → `setDirty` → setState。
这相当于在 useEditor 这一个 hook 执行过程中又触发了 re-render,导致同一个组件
的 hooks 注册顺序在两次 render 间被打乱,React 报 #310。

排查路径:看了好几轮自己写的 hook 顺序、conditional return、stale closure,
都没问题。最后查 Tiptap useEditor 源码 + GitHub issue 才定位。
...
const editor = useEditor({
  ...
  immediatelyRender: false,  // ★ 必加,React 18 兼容
  ...
})
```

附带做法:
- onUpdate 里调 setState 不会因此挂(已绕开 hook 注册期)
- 不需要自己写 useEffect 销毁 editor — useEditor 自带 cleanup
- hook 顺序:useState 先于 useEditor,这样 onUpdate 闭包引用 setState 时无 TDZ
