...
## 20. 场景驱动 / 蓝图回流一串坑(2026-07-16)

**20.1 改 `scene_*` 模型光改代码没用 —— DB 覆盖 + container 一次性脚本用的是静态路由,测了也白测(反复中招)**
`scene_reflow/scene_match/scene_ai_match/scene_questions` 的 primary/fallback 在 DB `agent_configs.routing_rules`,运行时覆盖 `model_router.py` 的 `ROUTING_RULES`(60s 缓存)。运维把它们全指到 `glm-5.1`(火山引擎,当时已坏)→ 前端点「手动识别蓝图回流」硬等 glm-5.1 的 3×ReadTimeout≈10min → 超 nginx 180s → **504,用户看不到任何东西**。**改法必须改 DB**:
```sql
UPDATE agent_configs SET config_value='{"primary":"minimax-m2.5","fallback":"minimax-m2.7"}'::json
WHERE config_type='routing_rules' AND config_key IN ('scene_reflow','scene_match','scene_ai_match','scene_questions');
```
**更阴的**:`docker compose exec backend python -c "..."` 一次性脚本调 `chat_with_routing` 走的是**静态代码路由(改后是 minimax),不吃 DB 覆盖**——所以在容器里测「通过」根本不代表前端路径通过(前端走 DB 的 glm-5.1)。这条让我反复「容器验证通过」但用户实际点了失败。**结论:验证 LLM 路由类改动,要么改完 DB 再测,要么直接打真 API,别信容器一次性脚本。** 同 §17。

**20.2 `scene_reflow` 缺 validator → 「回流永远出 0 条」**
同 §11.10/§17 家族:`propose_scene_changes` 调 `chat_with_routing` **没传 validator** → glm-5 思考烧光 token 吐空被静默接受 → `_extract_json_array("")→[]` → 提案永远 0(`scene_change_proposals` 表长期 0 行即此故)。修:加 `_reflow_output_valid`(截断/空→触发主备回退)+ 2 轮重试。**结构化 JSON 抽取一律带 validator + 用非推理模型 minimax-m2.5。**

**20.3 minimax 在「有依据地生成」上是跷跷板,靠 prompt 压不住两头**
配额压力(「挖全 5-15 条」)→ 抓关键词后**拿常识补细节**(编造蓝图没写的字段/参数,如「外勤签到照片/录音/位置」);反过来加「宁可少也别编 / 可能 0 条」→ 直接整体返回 `[]`。两版都翻车、来回 0↔28。落点:**prompt 只推召回**(挖全、别动不动返回空),**精度靠代码 + 人工**——每条要 `blueprint_evidence` 原文摘录、内容忠于蓝图别脑补,再交 PM + 后台两道人工闸门做减法。蓝图窗口喂全文(`_MAX_BLUEPRINT_CHARS=55000`)减少「后段没喂进去→拿常识补」。

**20.4 evidence 的字符串核对衡量的是「释义程度」不是「是否编造」**
想给提案自动标「已核对/待核」:精确子串 → 模型释义而非逐字抄,几乎全判 false(误报);改 6-gram 覆盖率 → 同批数据两次生成中位数 0.6 vs 0.3,阈值不稳,把大量「有据但被释义」误标待核。人工核过 evidence 里的专有对象名(`fault_case__c`/`competitor__c` 等)**真在蓝图里**——低覆盖是释义不是编造。**结论:别拿字符串相似度做「是否编造」的硬判定 / 绿橙标**,直接把原文出处展示给人自判更诚实。`evidence_verified` 仍在后端算存备查,但前端不据此告警。

**20.5 同步 LLM 端点 ~2 分钟 → 转异步 Celery + AsyncResult 轮询(可复用范式)**
回流读全文 + 产十几条富内容一次 ~130s,同步 HTTP 干等(虽在 nginx 180s 内但体验差、且接近超时)。转异步:`POST /scene-reflow` dispatch Celery task 立即返 `task_id`;`GET /scene-reflow/status/{task_id}` 查 `celery_app.AsyncResult(task_id).state`(**现成 redis result backend,`track_started=True` 能看到 STARTED,无需建表**);前端每 4s 轮询、ready 重拉列表、~6min 兜底不无限转。**这套 task_id 轮询范式适用于任何 >60s 的同步 LLM 端点**(scene_match ~30s 也可如法炮制)。
