...
M02_OPPORTUNITY = [
    BestPractice(
        ...
        triggers=["商机阶段", "阶段模型", "阶段推进", "漏斗", "赢率", "pipeline", "weighted"],
    ),
    BestPractice(
        title="阶段任务承载最佳实践",
        summary="每个阶段定义「必须完成的任务」,e.g. 方案阶段必须输出报价单 + 决策链梳理。任务勾选完才能推下一阶段(硬校验 / 软提醒可配)。",
        source_id="商机管理 / 标准流程 §4.3",
        triggers=["阶段任务", "checklist", "推进规则", "阶段必填", "硬校验", "软提醒"],
    ),
    BestPractice(
        title="可跳阶不可回退",
        summary="允许跳阶段前进(适应快单),但不允许回退,保证漏斗历史真实可审计。回退场景用「输单 → 重新立商机」处理。",
        source_id="商机管理 / 标准流程 §4.2",
        triggers=["阶段回退", "跳阶段", "阶段顺序"],
    ),
    BestPractice(
        title="终态数据锁定",
        summary="赢单 / 输单后填下单日期 + 输单理由(枚举:流标 / 输给竞品 / 价格原因 / 需求消失 ...),状态置终态 + 字段全部锁定,保证统计口径稳定。",
        source_id="商机管理 / 标准流程 §4.5",
        triggers=["赢单", "输单", "终态", "字段锁定", "数据锁定", "状态终态"],
    ),
    BestPractice(
        title="输单必复盘",
        summary="输单商机必须填关键事件 / 行动 / 原因 / 改进点,沉淀进知识库,为同类商机提供前车之鉴。",
        source_id="商机管理 / 标准流程 §4.6",
        triggers=["输单复盘", "输单原因", "商机复盘", "知识沉淀"],
    ),
    BestPractice(
        title="SMART 项目成功指标",
        summary="衡量实施成功用 SMART 框架:Specific(具体)/ Measurable(可量化)/ Achievable(可达成)/ Relevant(相关)/ Time-bound(有截止)。例:商机转化率 ≥ 22% by 2026.Q3、销售周期 ≤ 60 天 by 2026.Q4。避免「提升效率 / 数字化转型」这种无法验收的口号。",
        source_id="项目管理 / 成功标准定义 + 商机管理 §4",
        triggers=["SMART", "可量化", "成功指标", "成功标准", "成功的可量化", "项目目标", "项目成功", "KPI"],
    ),
    BestPractice(
        title="KPI 体系三层(战略 / 业务 / 操作)",
        summary="战略层(年度):营收 / 市占率 / NPS;业务层(季度):漏斗转化 / 客单 / 回款周期;操作层(月度 / 周):活动报告数 / 阶段任务完成率。CRM 看板按层做三档,各看各的,不互相打架。",
        source_id="项目管理 / KPI 体系 + 商机管理 §4 + 数据分析最佳实践",
        triggers=["KPI", "指标体系", "看板", "考核", "管理指标", "OKR"],
    ),
    BestPractice(
        title="实施验收三类标准",
        summary="技术验收(系统跑通 + 数据迁移率)、业务验收(关键流程 SOP 执行率 + 用户活跃)、价值验收(SMART 业务指标达标)。三层都过才算上线成功,避免「系统上线 = 项目成功」的错觉。",
        source_id="项目管理 / 验收标准 + 友发钢管 / 蒂业技凯项目档案",
        triggers=["验收", "上线标准", "项目成功", "成功标准", "项目验收", "上线判定"],
    ),
]
