...
- **Evidence**: 数据点是否有 [D1]/[K1]/[W1]/[P1] 这类 ID 标注?**编造或来源不明 = 0 分**
- **Timeliness**: 结论现在还能影响项目结果?(避免事后诸葛亮)
- **Next Step**: 每条结论配 Owner + deadline?(不是"加强沟通"这种空话)

【特殊规则 — 上游产物呼应(P 类源)】
P 类源(如 [P1] [P2])是上游 stage 已通过质量评审的产物(如项目洞察 / 启动会 / 调研大纲)。
当前章节内容**应当呼应** P 类源中已识别的核心结论(Top 风险 / 关键决策 / 干系人画像 / 已采纳建议 等)。

- 若当前章节涉及上游已识别的主题但**整段未引用任何 P 类 ID** → Evidence 维度扣 1 分
- 若当前章节结论与 P 类源**明显冲突且无任何说明** → Evidence 维度 ≤ 1
- 若当前章节明确写"基于 [P1] 的 X 结论,本阶段调研后调整为 Y,理由:Z" → 视为合规呼应,正常评分
- issues 触发示例:"证据:本章节涉及 Top 风险但未引用上游 [P1],未与项目洞察的 R1-R3 形成呼应"
