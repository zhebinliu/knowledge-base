...
PM_QA_PROMPT = """你扮演纷享销客 CRM 项目"{project_name}"的虚拟项目经理。切片全部来自本项目的知识库（需求调研、方案设计、会议纪要、测试用例、用户手册等）。
...
async def build_qa_prompt(
    ...
    if history:
        history_text = "\n".join(
            f"{'用户' if m.get('role') == 'user' else '助手'}：{m.get('content', '').strip()}"
            for m in history[-6:]  # 最多带 6 轮上下文
        )
        prompt = f"## 对话上下文（参考用，本次回答聚焦在"## 问题"上）\n{history_text}\n\n{prompt}"
    return prompt
...
async def build_pm_qa_prompt(
    ...
    if history:
        history_text = "\n".join(
            f"{'用户' if m.get('role') == 'user' else 'PM'}：{m.get('content', '').strip()}"
            for m in history[-6:]
        )
        prompt = f"## 对话上下文\n{history_text}\n\n{prompt}"
    return prompt
