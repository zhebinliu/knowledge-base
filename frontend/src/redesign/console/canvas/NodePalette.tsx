 ...
 *  - 拖拽到画布(HTML5 DnD)或点击添加;已在画布上的 kind 置灰(每 kind 至多一份)
 */
...
export default function NodePalette({ stageFlow, presentIds, onAdd, onLocate, onClose }: Props) {
  ...
  return (
    <div style={{
      ...
      borderRight: '1px solid var(--rd-line, rgba(255,255,255,0.08))',
      background: 'rgba(12,16,28,0.55)', backdropFilter: 'blur(8px)',
      padding: '12px 12px 24px',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--rd-text, #e8ecf5)' }}>节点库</span>
        <button className="nodrag" onClick={onClose} title="收起节点库"
          style={{ color: 'var(--rd-text-3, #94a3b8)', background: 'transparent', border: 'none', cursor: 'pointer', display: 'inline-flex' }}>
          <X size={15} />
        </button>
      </div>
      ...
      {groups.map(g => {
        ...
        return (
          <div key={g.stageKey}>
            <div style={{ fontSize: 10.5, color: 'var(--rd-text-3, #94a3b8)', margin: '12px 0 6px', letterSpacing: 1, display: 'flex', alignItems: 'center', gap: 5 }}>
