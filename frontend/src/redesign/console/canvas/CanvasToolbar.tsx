 ...
 *  节点库 / 全部添加 / 整理布局 / 一键全跑 / 删除选中 / 适应画布 / 还原 / 保存(+未保存指示)
...
const btn: React.CSSProperties = {
  ...
  background: 'var(--cv-chip-bg)', color: 'var(--cv-text)',
  cursor: 'pointer',
}
...
export default function CanvasToolbar(p: Props) {
  return (
    <div style={{
      ...
      <button
        style={{ ...btn, ...(p.hasSelection ? {} : disabledBtn), color: p.hasSelection ? '#ef4444' : undefined, borderColor: p.hasSelection ? 'rgba(239,68,68,0.4)' : undefined }}
      ...
      <span style={{ fontSize: 11, color: 'var(--cv-text-3)', marginLeft: 6 }}>
        {p.nodeCount} 节点 · {p.edgeCount} 连线
      </span>

      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
        {p.dirty && <span style={{ fontSize: 11, color: '#f59e0b' }}>未保存</span>}
        <button
